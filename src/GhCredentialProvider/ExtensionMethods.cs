...
{
    /// <summary>
    ///     Writes a <see cref="TraceEventType.Error" /> event message to the trace listeners in the
    ///     <see cref="TraceSource.Listeners" /> collection using the specified message.
    /// </summary>
    ...
    /// <summary>
    ///     Writes an <see cref="TraceEventType.Information" /> event message to the trace listeners in the
    ///     <see cref="TraceSource.Listeners" /> collection using the specified message.
    /// </summary>
    /// <param name="traceSource">A <see cref="TraceSource" /> instance to write the event to.</param>
    ...
    /// <summary>
    ///     Converts the current <see cref="Uri" /> with just the host by discarding the other parts like path and
    ///     querystrings.
    /// </summary>
    ...
    public static string? ToJsonWebTokenString(this string accessToken)
    {
        ...
        var start = accessToken.IndexOf('.') + 1;

        if (start <= 0) return null;

        var length = accessToken.IndexOf('.', start) - start;

        if (length <= 0 || start + length > accessToken.Length) return null;

        var base64String = accessToken.Substring(start, length);
        // Base64 requires padding to a multiple of 4
        var paddingNeeded = (4 - base64String.Length % 4) % 4;
        if (paddingNeeded > 0) base64String = base64String.PadRight(base64String.Length + paddingNeeded, '=');

        return Encoding.UTF8.GetString(Convert.FromBase64String(base64String));
    }

    /// <summary>
    ///     Writes a <see cref="TraceEventType.Verbose" /> event message to the trace listeners in the
    ///     <see cref="TraceSource.Listeners" /> collection using the specified message.
    /// </summary>
    /// <param name="traceSource">A <see cref="TraceSource" /> instance to write the event to.</param>
    /// <param name="message">The message to write.</param>
